  ...
  if (type == "VML") {
    var thePath = function(params, pathString, VML) {
      ...
      p.arcTo = function(rx, ry, large_arc_flag, sweep_flag, x2, y2) {
        ...
        var x1 = this.last.x,
            ...
            x = (x1 - x2) / 2,
            y = (y1 - y2) / 2,
            k = (large_arc_flag == sweep_flag ? -1 : 1) * Math.sqrt(Math.abs(rx * rx * ry * ry - rx * rx * y * y - ry * ry * x * x) / (rx * rx * y * y + ry * ry * x * x)),
      ...
      p.curveTo = function() {
        ...
        if (arguments.length == 6) {
          ...
          d += [Math.round(parseFloat(arguments[0], 10)), Math.round(parseFloat(arguments[1], 10)), Math.round(parseFloat(arguments[2], 10)), Math.round(parseFloat(arguments[3], 10)), Math.round(parseFloat(arguments[4], 10)), Math.round(parseFloat(arguments[5], 10))].join(" ") + " ";
          this.last.isAbsolute = this.isAbsolute;
        ...
        if (arguments.length == 4) {
          ...
          d += [Math.round(bx), Math.round(by), Math.round(parseFloat(arguments[0], 10)), Math.round(parseFloat(arguments[1], 10)), Math.round(parseFloat(arguments[2], 10)), Math.round(parseFloat(arguments[3], 10))].join(" ") + " ";
      ...
      p.qcurveTo = function() {
        ...
        if (arguments.length == 4) {
          ...
          this.last.x = (this.isAbsolute ? 0 : this.last.x) + parseFloat(arguments[2], 10);
          this.last.y = (this.isAbsolute ? 0 : this.last.y) + parseFloat(arguments[3], 10);
          d += [Math.round(this.last.qx), Math.round(this.last.qy), Math.round(this.last.x), Math.round(this.last.y)].join(" ") + " ";
          this.last.isAbsolute = this.isAbsolute;
    ...
    var setFillAndStroke = function(o, params) {
      ...
      if (typeof params.opacity != "undefined" || typeof params["stroke-width"] != "undefined" || typeof params.fill != "undefined" || typeof params.stroke != "undefined") {
        ...
        params["stroke-linecap"] && (stroke.endcap = {
          butt: "flat",
          square: "square",
          round: "round"
        }[params["stroke-linecap"]] || "miter");
      ...
      if (x != 0 && !(x == 1 && y == 1)) {
        ...
        if (dirx != 1 || diry != 1) {
          if (this.transformations) {
            ...
            this.node.filterMatrix = " progid:DXImageTransform.Microsoft.Matrix(M11=" + dirx + ", M12=0, M21=0, M22=" + diry + ", Dx=0, Dy=0, sizingmethod='auto expand', filtertype='bilinear')";
      ...
      (function() {
        ...
        if (time < ms) {
          for (var attr in from) {
            ...
            case "colour":
              now = "rgb(" + [
              Math.round(from[attr].r + time * diff[attr].r), Math.round(from[attr].g + time * diff[attr].g), Math.round(from[attr].b + time * diff[attr].b)].join(",") + ")";
              break;
            case "path":
              now = [];
              for (var i = 0, ii = from[attr].length; i < ii; i++) {
...
Raphael.getRGB = function(colour) {
  var red, green, blue, rgb = colour.match(/^\s*((#[a-f\d]{6})|(#[a-f\d]{3})|rgb\(\s*(\d+,\s*\d+,\s*\d+)\s*\)|rgb\(\s*(\d+%,\s*\d+%,\s*\d+%)\s*\)|hsb\(\s*(\d+,\s*\d+,\s*\d+)\s*\)|hsb\(\s*(\d+%,\s*\d+%,\s*\d+%)\s*\))\s*$/i);
  if (rgb) {
    if (rgb[2]) {
