...
nfn.ui.view.BugsHelper = nfn.ui.view.Widget.extend({
  ...
  toggleText: function() {
    ...
    var widget = this.parent.transcriberWidget.$el;
    var that = this;
    ...
    if (this.model.get('showText')) {
      ...
      this.$toggle.fadeOut({
        complete: function() {
          ...
          position.left = position.left + parseInt(widget.css('margin-left'));
  ...
  createTooltip: function(e) {
    ...
    y           = Math.abs(this.$el.offset().top  - this.$exampleLink.offset().top) - this.tooltip.$el.height() + 30;
