...
nfn.ui.view.BugsWidget = nfn.ui.view.Widget.extend({
  ...
  initialize: function() {
    _.bindAll( this, "toggle", "toggleOk", "onEnter", "updatePlaceholder", "updateInputTitle", "updateValue", "updateType", "createStepTooltip", "closeTooltip", "closeErrorTooltip", "closeFinishTooltip", "closeStepTooltip", "gotoStep" );
  ...
  updateType: function() {
    ...
    var that = this;
    this.$input.autocomplete({    delay: 0, minLength: 0,	
        source: function( request, response ) {
        	...
        	for (var key in cache) {
        		...
        		var matches = retrieveClassificationEntries(entry, that.$input.data("title"));
        		if (matches.length > 0) {
        			var value = matches[0];
        ...
        search: function(event,ui) {
        	if (event.keyCode || !event.isTrigger) {	// only open the menu for the ctrl-p input detected by TranscriptionController
        		event.preventDefault();
        	}
        },
        select: function( event, ui ) {
        	if (jQuery.inArray(that.$input.data("title"), ['Begin Date Collected','End Date Collected']) > -1) {  // TODO use a better identifier?
        		splitValue(ui.item.value, $(".ui-autocomplete-input"), "/");
            	event.preventDefault();
        	}
        }
    });
  ...
  render: function() {
    ...
    function retrieveClassificationEntries(classification, fieldName) {
    	var merged = [];
    	merged = merged.concat.apply(merged,
    			jQuery.map(classification.classification.annotations, 
	    			function(element) { 
    ...
    function splitValue(value, targetEls, delimiter) {
