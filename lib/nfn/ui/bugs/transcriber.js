...
nfn.ui.view.Bugs = nfn.ui.view.Transcriber.extend({
  initialize: function() {
    ...
    this.guide = [
      {
        ...
        description: 'This is the day on which the specimen was collected, or a period of time during which the specimen could have been collected. For example, a single specimen collected using a net by a researcher on 6 July 2011 would only have that date, while one of many specimens collected in a long duration trap might have a range of dates like 12 June – 5 July 2010. Entomologists use many different date formats, but one the most common is having the month be a Roman numeral, like “3.iv.1987” for April 3rd 1987. Sometimes date information is incomplete. If there is a range of dates, enter the beginning date in the boxes below, and the ending date on the next screen. If there is just one date, enter it into the "Begin Date Collected" fields below and leave the "End Date Collected" fields empty. <a href="#" class="example">See example</a>',
        ...
        description: 'This is the end day in a period of time during which the specimen could have been collected. Conventions are identical to other date information on the label. If there is only one date on the label, please leave this field blank.',
        examples: ['nfn/ui/bugs/examples/date-collected.jpg'],
        placeholder: ['day', 'month', 'year'],
        type: 'date',
        inputWidth: 700,
    ...
    this.model.bind("change:currentRecord", function(model) {
      var counterText = (model.get('currentRecord') == 1) ? 'Record Done': 'Records Done';
      that.statusBar.$counter.text(model.get('currentRecord'));
      that.statusBar.$cText.text(counterText);
    });
