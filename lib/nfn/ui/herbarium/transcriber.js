...
nfn.ui.view.Herbarium = nfn.ui.view.Transcriber.extend({
  ...
  startTranscribing: function() {
    ...
    currentX = (window.innerWidth / 2) - 250;
    currentY = window.innerHeight - this.transcriberWidget.height() - padding;

    // Add the transcription widget
    this.transcriberWidget.setWidth(500).setPosition(currentX, currentY);
    this.transcriberWidget.show();

    currentY = currentY - this.helper.height() - padding;

    // Add the Helper button
    this.helper.setWidth(500).setPosition(currentX, currentY);
    this.helper.show();
  ...
  render: function() {
    ...
    // this.$el.append('<div class="photos" id="photo-container" />');
