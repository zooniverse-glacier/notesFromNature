...
nfn.ui.view.Herbarium = nfn.ui.view.Transcriber.extend({
  ...
  addViews: function() {
    ...
    var title = this.archive.metadata.title,
        description = '';
    if (this.archive.metadata.collection_name) {
        title = this.archive.metadata.collection_name;
        description = "from the " + this.archive.metadata.title;
    }
  ...
  render: function() {
    ...
    if(user) {
      ...
      if(('preferences' in user) && (project.id in user.preferences) && ('plants_popup' in user.preferences[project.id])) {
        this.seenPopup = true;
      }
    }
