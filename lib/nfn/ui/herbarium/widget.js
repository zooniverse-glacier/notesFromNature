...
nfn.ui.view.HerbariumWidget = nfn.ui.view.Widget.extend({
  ...
  ok: function(e) {
    ...
    if ((type == undefined || type == 'text' || type == 'location' || type == 'state' || type == 'county') && this.$input.val()) {
      isEmpty = false;
    } else if (type == 'date') {
  ...
  resize: function() {
    ...
    if ( type == 'text' || type == 'location' || type == 'state' || type == 'county') {
  ...
  updateType: function() {
    ...
    if (type == 'county') {
      ...
      var state = this.parent.getStepData(0).get('value')
