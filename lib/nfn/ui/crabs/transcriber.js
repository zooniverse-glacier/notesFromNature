// CRAB TRANSCRIBER ------------------------------------------
nfn.ui.model.Crab = nfn.ui.model.Transcriber.extend({
  defaults: {
    type: 'crab',
    visible: true
  }
...
nfn.ui.view.Crab = nfn.ui.view.Transcriber.extend({
  initialize: function() {
    var that = this;
    ...
    this.guide = [{
        ...
        description: 'A number (or code) assigned to the specimen in the field, usually found after the Collector’s name. <a href="#" class="example">See example</a>',
        placeholder: 'Number',
        type: 'text',
        inputWidth: 550
      }, {
  ...
  positionWidgets: function(position) {
    var padding = 10, currentX, currentY;
  ...
  addViews: function() {
    ...
    this.transcriberWidget = new nfn.ui.view.HerbariumWidget({
      ...
      template: $("#transcriber-crab-widget-template").html(),
      parent: this
    });
  ...
  render: function() {
    ...
    if(user) {
      ...
      if(('preferences' in user) && (project.id in user.preferences) && ('crabs_popup' in user.preferences[project.id])) {
        this.seenPopup = true;
      }
    }
    ...
    if(!this.seenPopup) {
      ...
      if(user) {
        user.setPreference('crabs_popup', true, function() {});
      }
    }
